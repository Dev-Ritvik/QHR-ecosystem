...
| 30 | Ticker teardown | `stopTicker()` called `state.subs.clear()` | **Removed.** WorldCanvas owns the ticker and is `next/dynamic`, so it mounts after ExperienceHost has already subscribed; StrictMode's start/stop/start then wiped the audio subscription. 4 live subscribers where there should have been 6 — the drone held one pitch all scroll. Gated by `rig-check.mjs` |
| 31 | Ticker `dt` | unclamped `gsap.ticker.deltaRatio()` | **Clamped finite, 100 ms ceiling.** Two subscribers *integrate* dt (`chaseExposure`, the camera-speed smoother), so one bad delta poisons their state permanently — a NaN made `toneMappingExposure` NaN for the rest of the session |
| 32 | Chromatic aberration | constant 0.0006 across the whole timeline | **Velocity-driven, hard zero at rest.** A static offset splits every 1-pixel survey line in the Act II grid into red and blue ghosts, and accounted for 100% of Act I's residual warm pixels. Now `caOffsetFor(speed)` — exactly 0 below 8 m/s, 0.0008 through the punch |
| 33 | Act I legibility | EV alone was blamed | **Both.** EV 0 lifted −2.40 → −1.60, AND the terrain ambient raised 0.16+0.44 → 0.34+0.68. Exposure alone moved only the ceiling: measured mean stayed at 23 with 88–99% of the frame in one perceptual band. The frame was not underexposed, it was FLAT |
